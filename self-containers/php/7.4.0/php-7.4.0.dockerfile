...
ENV DIR_PHP_EXTENSIONS "/usr/local/lib/php/extensions/no-debug-non-zts-20190902"
...
#COPY ./conf/library/postgres/*.deb /opt/postgres/
#RUN apt install -y /opt/postgres/libffi7_3.3-4_amd64.deb
#RUN apt install -y /opt/postgres/php-common_75_all.deb
#RUN apt install -y /opt/postgres/php7.4-common_7.4.3-4ubuntu2.19_amd64.deb
#RUN apt install -y /opt/postgres/php7.4-pgsql_7.4.3-4ubuntu2.19_amd64.deb
...
COPY ./ini/php/php.ini $DIR_PHP_INI/php.ini
COPY ./ini/*.ini $DIR_PHP_INI_FILES/

COPY ./extensions/php-7.4.0-compiled-extensions.zip $DIR_PHP_EXTENSIONS/

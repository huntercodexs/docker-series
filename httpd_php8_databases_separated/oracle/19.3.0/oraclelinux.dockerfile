...
USER root
RUN $INSTALL_DIR/$INSTALL_DB_BINARIES_FILE $DB_EDITION

USER root
#RUN rm $INSTALL_DIR/$INSTALL_FILE_1
...
# Database create automatic
CMD exec $ORACLE_BASE/$CREATE_DB_FILE

# Set password to oracle administrator (from .env file)
CMD exec $ORACLE_BASE/$PWD_FILE $ORACLE_PASSWORD

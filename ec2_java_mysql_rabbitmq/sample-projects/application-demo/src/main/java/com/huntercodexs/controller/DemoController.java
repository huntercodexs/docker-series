...
public class DemoController {
    ...
    @Autowired
    OrderService orderFileService;

    @GetMapping(path = "/application-demo/api/v1/welcome")
    @ResponseBody
    ...
    @PostMapping(
            path = "/application-demo/api/v1/order/create",
            consumes = {
    ...
    public ResponseEntity<?> create(@RequestBody(required = false) JSONObject jsonDataRequest) {
        ...
        String id = orderFileService.save(jsonDataRequest.getAsString("order"));
    ...
    @GetMapping(path = "/application-demo/api/v1/order/read/{orderId}")
    @ResponseBody
    public ResponseEntity<?> read(String order, @PathVariable(value = "orderId", required = false) String orderId) {
